...
import com.longoj.top.model.dto.user.*;
import com.longoj.top.model.entity.User;
import com.longoj.top.model.vo.LoginUserVO;
...
import static com.longoj.top.service.impl.UserServiceImpl.SALT;
...
public class UserController {
    ...
    public BaseResponse<String> userLogin(@RequestBody UserLoginRequest userLoginRequest, HttpServletRequest request) {
        ...
        // 上面记录了Session，这里补充一个Token
        String token = JwtTokenUtil.generateToken(loginUserVO.getId());
        return ResultUtils.success(token);
    }

    @ApiOperation("用户登录-token")
    @PostMapping("/login/token")
    public BaseResponse<String> userLoginWithToken(@RequestBody UserLoginRequest userLoginRequest) {
        if (userLoginRequest == null) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR);
        }
        String userAccount = userLoginRequest.getUserAccount();
    ...
    public BaseResponse<LoginUserVO> getLoginUserWithToken(HttpServletRequest request) {
        String token = request.getHeader(JwtTokenUtil.tokenHeader);
        if (token == null || !JwtTokenUtil.validateToken(token)) {
            ResultUtils.error(ErrorCode.NO_AUTH_ERROR);
        }
        Long loginUserId = JwtTokenUtil.getUserIdFromToken(token);
    ...
    public BaseResponse<Boolean> updateUserPwd(@RequestBody UserUpdatePwdRequest userUpdatePwdRequest,
        ...
        boolean ok = userService.updateUserPwd(request, userUpdatePwdRequest);
        if (ok) {
